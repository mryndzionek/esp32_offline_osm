...
#define OFFSET (-(TILE_SIZE - 240) / 2)
    ...
    while (1)
    {
        for (uint8_t z = 0; z <= MAX_ZOOM_LEVEL; z++)
        {
            ...
            r = get_file_name(filename, sizeof(filename), z, x, y);
            if (r)
            {
                if (pdTRUE == xSemaphoreTake(xGuiSemaphore, portMAX_DELAY))
                {
                    ...
                    lv_obj_align(line1, LV_ALIGN_CENTER, dx + OFFSET - (TILE_SIZE / 2), dy + OFFSET - (TILE_SIZE / 2));
                    lv_obj_align(line2, LV_ALIGN_CENTER, dx + OFFSET - (TILE_SIZE / 2), dy + OFFSET - (TILE_SIZE / 2));
                    lv_obj_align(tile, LV_ALIGN_CENTER, OFFSET, OFFSET);
